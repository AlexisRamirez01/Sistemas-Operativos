...
char * scommand_to_string(const scommand self){
	...
	//Creo este arreglo para ir guardando los largos de las cadenas de los comandos
	...
	//Si no hay comando, retorno un string vacío 
	if (res_length == 0)
	{
		free(chain_length);
...
char * pipeline_to_string(const pipeline self){
	...
	//Si no hay pipe devuelvo un string vacío 
	if (self->length == 0)
	{
		return calloc(1, sizeof(char));
	}

	//Guardo todos los comandos
	...
	//Veo si la pipeline requiere ir a ejecutarse en background
