...
static void execute_command (scommand comm){
  ...
  unsigned int length_comm = scommand_length (comm); // cant argumentos
  unsigned int comml = strlen (scommand_front (comm)) + 1; //long del argumento
  char **argv = calloc (length_comm + 1, sizeof (char *)); // este es el espacio de mi cadena/arreglo para guardar el camando
  unsigned int i = 0; // i es el indice para irme moviendo en el arreglo
  char * redir_out= scommand_get_redir_out(comm);  // fork ¿?¿?
  char * redir_in = scommand_get_redir_in(comm);  //aprende usar dup ()

  if (scommand_is_empty (comm)) {
    fprintf (stderr, "Porfavor inserte un comando.\n"); //borrar dsp, dif frprint no afecta la salida del directorio
    return;
  }

  
  while (!scommand_is_empty(comm)){
    argv [i] = calloc (comml , sizeof(char*)); // asigno la memoria para el comm
    strcpy (argv [i], scommand_front (comm) ); // copio con nulo, (destino, origen)
...
void execute_pipeline(pipeline apipe) {
  ...
  unsigned int pipelength = pipeline_length (apipe); // cuantos comandos tengo 
  ...
  //Creo este arreglo para ir guardando los id de los procesos hijos

  int tmp[2];
  int fileno[2];

  /*char * view = pipeline_to_string(apipe);
  printf("%s\n", view);
  */
